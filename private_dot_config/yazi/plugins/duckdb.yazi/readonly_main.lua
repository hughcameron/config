...
local function generate_preload_query(job, mode, file_type, limit)
	...
	if mode == "standard" then
		...
		-- LOCAL PATCH: bound the summarize pass with the same limit.
		-- Upstream summarizes the whole file, so every preload pays a full scan
		-- computing count/unique/min/max/avg/std no matter what cache_size says.
		-- Summarizing the head is a weaker claim -- stats over the first N rows,
		-- not the file -- but it keeps preview cost flat on large inputs.
		return string.format(
			"SELECT * EXCLUDE(null_percentage), CAST(null_percentage AS DOUBLE) AS null_percentage FROM (SUMMARIZE FROM (FROM %s%s))",
			data_source_string,
			limit_string
		)
	end
